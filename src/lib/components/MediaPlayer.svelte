...
<div class="flex flex-col h-full bg-night-black/40 font-jetbrains select-none">
  ...
  <div class="h-44 relative border-b border-white/5 overflow-hidden">
    <canvas bind:this={canvas} width="400" height="176" class="w-full h-full opacity-30"></canvas>
    <div class="absolute inset-0 flex flex-col items-center justify-end pb-6 px-6 bg-gradient-to-t from-night-black/90 via-night-black/40 to-transparent">
      <h3 class="text-white font-bold tracking-[0.2em] uppercase text-sm truncate w-full text-center drop-shadow-lg">
        {playlist[currentIndex].title}
      </h3>
      <p class="text-night-pink text-[10px] uppercase tracking-[0.3em] opacity-80 mt-1 text-center">
  ...
  <div class="p-4 space-y-4">
    ...
    <div class="space-y-2 px-2">
      <div class="relative w-full h-1.5 bg-white/5 rounded-full overflow-hidden group/progress">
        <!-- Progress Fill -->
        <div 
      ...
      <div class="flex justify-between text-[9px] font-bold text-white/30 uppercase tracking-[0.2em]">
        <span class="{isPlaying ? 'text-night-pink/60' : ''}">{formatTime(currentTime)}</span>
    ...
    <div class="flex items-center gap-3 px-4">
      <button onclick={() => isMuted = !isMuted} class="text-white/40 hover:text-white transition-colors">
        {#if isMuted || volume === 0}
  ...
  <!-- Playlist -->
  <div class="flex-grow overflow-y-auto custom-scrollbar border-t border-white/5 bg-night-black/20">
    {#each playlist as track, i}
      <button 
        onclick={() => handleTrackSelect(i)}
        class="w-full px-5 py-4 flex items-center gap-4 hover:bg-white/5 transition-colors group {currentIndex === i ? 'bg-white/5' : ''}"
      >
        <div class="w-10 h-10 rounded-sm bg-white/5 border border-white/5 flex items-center justify-center text-white/20 group-hover:text-night-pink group-hover:border-night-pink/30 transition-all relative overflow-hidden">
          {#if currentIndex === i}
            {#if isPlaying}
              <Pause size={18} weight="fill" class="text-night-pink animate-pulse" />
              <!-- Subtle equalizer overlay -->
              <div class="absolute bottom-1 flex gap-0.5 items-end h-2 opacity-40">
                <div class="w-0.5 bg-night-pink animate-[music-bar_0.8s_infinite]"></div>
                <div class="w-0.5 bg-night-pink animate-[music-bar_1.2s_infinite]"></div>
                <div class="w-0.5 bg-night-pink animate-[music-bar_1s_infinite]"></div>
              </div>
            {:else}
        ...
        <div class="flex flex-col items-start justify-center overflow-hidden flex-grow text-left">
          <span class="text-[13px] font-bold text-white group-hover:text-night-pink transition-colors truncate w-full text-left {currentIndex === i ? 'text-night-pink' : ''}">
            {track.title}
          </span>
          <span class="text-[9px] text-white/40 uppercase tracking-[0.2em] mt-0.5 truncate w-full text-left group-hover:text-white/60 transition-colors">
            {track.artist}
          </span>
        </div>
      </button>
